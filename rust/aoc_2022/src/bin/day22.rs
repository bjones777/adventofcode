        ...
        match path_char {
            'L' => {
                if !to_move.is_empty() {
                    cl = follow_path(
                        &monkey_map,
                        &row_ranges_of_columns,
                        &col_ranges_of_rows,
                        cl,
                        cur_dir,
                        to_move.parse::<i32>().unwrap(),
                    );
                }
                to_move.clear();
...
impl MonkeyMap {
    ...
    fn adjust_for_region(&self, a: (usize, usize), cur_dir: Dir) -> ((usize, usize), Dir) {
        ...
        (
            self.get_global_coord(new_local_coord, new_region),
            new_direction,
        )
    }
    ...
    for path_char in path.chars() {
        match path_char {
            'L' => {
                if !to_move.is_empty() {
                    (cl, cur_dir) =
                        cube_monkey_map.follow_path(cl, cur_dir, to_move.parse::<i32>().unwrap());
                }
                to_move.clear();
                cur_dir = turn_left(cur_dir);
            }
