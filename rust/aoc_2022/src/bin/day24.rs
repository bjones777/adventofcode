        ...
        match blizzard.dir {
            Dir::East => {
                if board[blizzard.loc.0][blizzard.loc.1 + 1] == TileType::Wall {
                    moved_blizzards.push(Blizzard {
                        loc: (blizzard.loc.0, 1),
                        dir: blizzard.dir,
                    });
                } else {
                    moved_blizzards.push(Blizzard {
                        loc: (blizzard.loc.0, blizzard.loc.1 + 1),
                        dir: blizzard.dir,
                    });
                }
            }
            Dir::West => {
                if board[blizzard.loc.0][blizzard.loc.1 - 1] == TileType::Wall {
                    moved_blizzards.push(Blizzard {
                        loc: (blizzard.loc.0, board[blizzard.loc.0].len() - 2),
                        dir: blizzard.dir,
                    });
                } else {
                    moved_blizzards.push(Blizzard {
            ...
            Dir::South => {
                if board[blizzard.loc.0 + 1][blizzard.loc.1] == TileType::Wall {
                    moved_blizzards.push(Blizzard {
                        loc: (2, blizzard.loc.1),
                        dir: blizzard.dir,
                    });
                } else {
                    moved_blizzards.push(Blizzard {
                        loc: (blizzard.loc.0 + 1, blizzard.loc.1),
                        dir: blizzard.dir,
                    });
                }
            }
            Dir::North => {
                if board[blizzard.loc.0 - 1][blizzard.loc.1] == TileType::Wall {
                    moved_blizzards.push(Blizzard {
                        loc: (board.len() - 3, blizzard.loc.1),
                        dir: blizzard.dir,
                    });
                } else {
                    moved_blizzards.push(Blizzard {
...
fn enqueue(
    in_q: &mut HashSet<((usize, usize), usize)>,
    queue: &mut VecDeque<((usize, usize), usize)>,
    new_state: ((usize, usize), usize),
) {
...
fn is_loc_safe(
    ...
    blizzards_at_round: &mut Vec<Vec<Blizzard>>,
    blizzard_locs_at_round: &mut Vec<HashSet<(usize, usize)>>,
    round: usize,
) -> bool {
...
fn get_rounds_to_reach_dest(
    ...
    while !queue.is_empty() {
        ...
        if is_loc_safe(
            ...
            &board,
            blizzards_at_round,
            blizzard_locs_at_round,
            state.1 + 1,
        ) {
            enqueue(&mut in_q, &mut queue, (state.0, state.1 + 1));
        }
        if board[loc.0 - 1][loc.1] == TileType::Empty
            && is_loc_safe(
                (loc.0 - 1, loc.1),
                &board,
                blizzards_at_round,
        ...
        if board[loc.0][loc.1 + 1] == TileType::Empty
            && is_loc_safe(
                (loc.0, loc.1 + 1),
                &board,
                blizzards_at_round,
...
fn part_a(lines: &Vec<String>) -> usize {
    ...
    get_rounds_to_reach_dest(
        start_loc,
        end_loc,
        0,
        &mut blizzards_at_round,
        &mut blizzard_locs_at_round,
        &board,
    )
}
...
fn part_b(lines: &Vec<String>) -> usize {
    ...
    let rounds_to_end = get_rounds_to_reach_dest(
        start_loc,
        end_loc,
        0,
        &mut blizzards_at_round,
    ...
    let rounds_to_start = get_rounds_to_reach_dest(
        end_loc,
        start_loc,
        rounds_to_end + 1,
        &mut blizzards_at_round,
