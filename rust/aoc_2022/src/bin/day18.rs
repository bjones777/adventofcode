...
fn part_b(lines: &Vec<(i32, i32, i32)>) -> i32 {
    ...
    loop {
        ...
        for t in water_set.iter() {
            if !rock_set.contains(&(t.0 - 1, t.1, t.2))
                && !water_set.contains(&(t.0 - 1, t.1, t.2))
                && inbounds(&(t.0 - 1, t.1, t.2), &min_coords, &max_coords)
            {
            ...
            if !rock_set.contains(&(t.0, t.1 - 1, t.2))
                && !water_set.contains(&(t.0, t.1 - 1, t.2))
                && inbounds(&(t.0, t.1 - 1, t.2), &min_coords, &max_coords)
            {
            ...
            if !rock_set.contains(&(t.0, t.1, t.2 - 1))
                && !water_set.contains(&(t.0, t.1, t.2 - 1))
                && inbounds(&(t.0, t.1, t.2 - 1), &min_coords, &max_coords)
            {
