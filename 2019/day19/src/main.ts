    ...
    for(let x = 0;x < 50; ++x) {
      ...
      s.setOutputCallback((v : bigint) => {
        if(v === 0n) {
          ...
          throw new Error(`What is this ${v}`);
        }
        if(line.length >= 50) {
          console.log(line);
          line = "";
      ...
      s.pushInput(BigInt(x));
      s.pushInput(BigInt(y));
      s.runSimInternal();
    }
  }
  console.log(sum);
...
function testXY(x : number,y : number) : boolean {
  let retVal = false;
  let s = new Simulator(arr);
  s.setOutputCallback((v : bigint) => {
    if( v=== 1n) {
...
function partB() : number {
  ...
  for(let y = 99;;++y) {
    ...
    if(testXY(x,y-99) && testXY(x+99,y) && testXY(x+99,y-99)) {
      console.log(`(${x},${y-99})`);
      return x * 10000 + (y-99);
    }
  }
